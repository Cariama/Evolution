...
def recup() :
    ...
    A = Evol(nombre_de_creature,largeur,hauteur,rapport_nourriture=rapport_bouffe,t_day=day,rapport_predation= rp, V1 = variable_1.get(), V2=variable_2.get(), V3 = variable_3.get())
    B = display(A)
    B.launch()
                    ...
                    for i in range(self.Nod) :
                        ...
                        while self.class_in.t < self.class_in.t_day-1 :
                            ...
                            if (self.class_in.day%self.factor) == 0 :
                                ...
                                canvas.create_text(100, 50, text="hour : {}\nday : {}\npopulation : {}\ngeneration : {}".format(self.class_in.t,self.class_in.day,len(self.class_in.creatures),int(mean(list_of_gen)*100+0.5)/100), font="Arial 16 italic", fill="black")
                                canvas.update()
                        y = len(self.class_in.creatures)
                        self.ax1.plot(self.x,y,"g^")
                        speed_list = []
...
but_V = Button(window,text="Valider", command=recup)
but_V.pack()
