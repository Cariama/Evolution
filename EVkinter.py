...
class display :
    ...
    def launch(self) :
        ...
        for i in self.class_in.creatures :
            canvas.create_oval((i.position[0]-i.hitbox)*100*self.scale,(i.position[1]-i.hitbox)*100*self.scale,(i.position[0]+i.hitbox)*100*self.scale,(i.position[1]+i.hitbox)*100*self.scale,fill = "#555555")
            ...
            if touche == "s":
                for i in range(self.Nod) :
                    ...
                    while self.class_in.t < self.class_in.t_day :
                        ...
                        time.sleep(0.05)
                        canvas.create_text(50, 20, text="hour : {}\nday : {}".format(self.class_in.t, self.class_in.day), font="Arial 16 italic", fill="black")
                        canvas.update()
        canvas.focus_set()
